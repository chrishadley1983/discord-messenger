# Monthly deliberate Windows Update install (portable pack version).
...
# verified the component store is clean.
...
$lastHealth = Get-ChildItem $logDir -Filter 'health_*.log' -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if ($lastHealth) {
...
if ($failed.Count -gt 0) {
    ...
    $rebootMsg = if ($rebootNeeded) { "**Reboot needed - please restart TODAY (daytime), don't leave it pending overnight.**" } else { 'No reboot required.' }
