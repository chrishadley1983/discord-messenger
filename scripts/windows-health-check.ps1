# Monthly Windows component-store health check.
# Runs DISM ScanHealth + SFC before Patch Tuesday so store corruption is caught
# BEFORE a cumulative update lands on it (see memory: incident-july-cu-corruption —
# the Jul 2026 CU installed onto a corrupt store and took out networking on two machines).
# Posts a one-line green/red summary to Discord #alerts (webhook from repo .env).
# Runs as SYSTEM via the WindowsHealthCheck-Monthly scheduled task; elevation required.
