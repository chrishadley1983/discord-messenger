...
export default function DadJokeCard({ jokes }: DadJokeCardProps) {
  ...
  if (count === 0) {
    return (
      <div className="dadjoke-chip-rotate h-full min-h-0 flex flex-col">
        <Card section="kids" chip="Peter Says" chipIcon={<Icon name="speech" size={16} />} className="flex-1 min-h-0">
          <div className="flex-1 min-h-0 flex flex-col items-center justify-center gap-2 text-center">
            <Icon name="speech" size={36} style={{ color: "var(--ink-30)" }} />
  ...
  return (
    <div className="dadjoke-chip-rotate h-full min-h-0 flex flex-col">
      <Card
        section="kids"
        chip="Peter Says"
        chipIcon={<Icon name="speech" size={16} />}
