...
def get_japan_context(sim_date: str | None = None) -> str:
    ...
    parts = []
    if date_override:
        parts.append("## ⚠️ SIM MODE ACTIVE")
        parts.append(f"Treat this conversation as if the family IS in Japan right now on Day {trip_day}.")
        parts.append("Respond as their on-the-ground trip concierge. Do NOT mention that the trip hasn't happened yet.")
        parts.append("")
    ...
    parts.append("This scrapes JR West/East live status pages and returns JSON with per-area status.")
    parts.append("Report the results clearly — which lines are normal, which have delays.")
    parts.append("If a line is disrupted, suggest alternative routes.")
    parts.append("")
    ...
    # Photo book pipeline — jd/ command
    parts.append("### Photo Book — `jd/` Command")
    parts.append("We're building a post-trip photo book. The user saves content with the `jd/` prefix.")
    parts.append("ONLY act on messages starting with `jd/` — never auto-capture from normal conversation.")
    parts.append("")
    parts.append("**When a message starts with `jd/`, work out which type it is and save it:**")
    parts.append("")
    parts.append("**Photo** — `jd/` with an image attachment (caption is the text after `jd/`, or describe what you see):")
    parts.append('```bash')
    parts.append('curl -s -X POST http://172.19.64.1:8100/japan/photobook/upload -H "Content-Type: application/json" -d \'{"local_path": "IMAGE_LOCAL_PATH", "caption": "CAPTION", "sender": "SENDER_NAME"}\'')
    parts.append('```')
    ...
    parts.append("- Reply: `📸 Day {X}` and a short acknowledgement")
    parts.append("")
    parts.append("**Highlight** — `jd/` followed by a short message (1-2 sentences), e.g. `jd/ Emmie lost her mind at the deer`:")
    parts.append('```bash')
    parts.append('curl -s -X POST http://172.19.64.1:8100/japan/photobook/highlight -H "Content-Type: application/json" -d \'{"text": "THE TEXT AFTER jd/", "sender": "SENDER_NAME"}\'')
    parts.append('```')
    parts.append("- Reply: `✨ Day {X}` and a short acknowledgement")
    parts.append("")
    parts.append("**Diary** — `jd/` followed by a longer message (3+ sentences), e.g. `jd/ Today was incredible. We started at...`:")
    parts.append('```bash')
    parts.append('curl -s -X POST http://172.19.64.1:8100/japan/photobook/diary -H "Content-Type: application/json" -d \'{"content": "THE TEXT AFTER jd/", "source": "text", "sender": "SENDER_NAME"}\'')
    parts.append('```')
    parts.append("- Reply: `📝 Day {X} — {N} words`")
    parts.append("")
    parts.append("**Voice note** — `jd/` with a voice note attachment (transcription appears as `[Voice note transcription]: ...`):")
    parts.append('```bash')
    parts.append('curl -s -X POST http://172.19.64.1:8100/japan/photobook/diary -H "Content-Type: application/json" -d \'{"content": "TRANSCRIBED_TEXT", "source": "voice_note", "sender": "SENDER_NAME"}\'')
    parts.append('```')
    parts.append("- Reply: `🎤 Day {X} — {N} words`")
    parts.append("")
    parts.append("Keep replies SHORT — just the emoji + day + brief confirmation. No chatter.")
    parts.append("")
