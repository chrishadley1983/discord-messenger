...
def get_japan_context(sim_date: str | None = None) -> str:
    ...
    parts.append("If asked about train delays, CHECK LIVE STATUS. Run this command from the japan-family-guide directory:")
    parts.append("```bash")
    parts.append('cd /mnt/c/Users/"Chris Hadley"/claude-projects/japan-family-guide && node -e "const{chromium}=require(\'playwright\');(async()=>{const b=await chromium.launch({headless:true});const p=await b.newPage();await p.goto(\'https://trafficinfo.westjr.co.jp/kinki.html\',{waitUntil:\'networkidle\',timeout:15000});await p.waitForTimeout(2000);const t=await p.evaluate(()=>document.body.innerText);console.log(t.substring(0,800));await b.close()})()"')
    parts.append("```")
    parts.append("Parse the Japanese output: 平常運転 = Normal, 遅延あり = Delays, 運転見合わせ = Suspended")
    parts.append("For Tokyo, change the URL to: https://traininfo.jreast.co.jp/train_info/e/")
    parts.append("Report results clearly in English. If delays, suggest alternative routes.")
    parts.append("")
