...
Morning briefing covering TODAY and TOMORROW for Max and Emmie. Combines calendar events, recurring activities, school notices, spellings, and kit reminders into one scannable message.
...
Read `activities.json` from this skill's directory for recurring activities and children info.
...
# Calculate tomorrow's date and fetch
...
📌 **TODAY**
👦 Max (Year 6): School → Swimming 4pm 🏊
...
⚠️ **Heads up:** [anything special about tomorrow]
```
...
2. Look up recurring activities from `activities.json` for that day
3. Merge with any one-off calendar events for today (birthdays, trips, playdates)
4. For each child, build their day: "School → [activity] [time] [emoji]"
5. If no after-school activity: "School as normal → pickup {normal_pickup}"

### Determining kit needed

1. Check today's recurring activities for any with `kit` specified
2. Check tomorrow's activities for kit that needs packing tonight
3. If swimming is tomorrow, remind to pack tonight
4. Per-child if activities differ (e.g., one has tutoring, other has swimming)

### Weekend handling

- Saturday/Sunday: Only show non-school activities (Piano, Tutoring, etc.)
- Don't show "School" prefix on weekends
- If no activities: "No activities today — enjoy the day off!"

### School holidays / INSET days

- If detected (from school data), replace school activities with:
  "🏖️ No school today — [holiday name / INSET day]"
- Still show any non-school activities (Piano, etc.)

## Conversational Follow-ups

- "What kit does Max need tomorrow?" → Check tomorrow's activities
- "Does Emmie have tutoring this week?" → Check activity config
...
- Use the emojis from activities.json for each activity type
