...
- [x] ✅ [RESOLVED 25 Jul] **finance schema was readable with the public anon key — FIXED and verified.** Applied as two migrations: `revoke_anon_access_to_finance_schema` and `revoke_anon_execute_on_finance_manual_income`. Approach differs from the SQL drafted below: rather than rewriting policies on 3 tables, it revoked anon's table privileges across the whole schema, which closes all of them at once. Also revoked anon from `pg_default_acl` (it was `{anon=r/postgres}`, so **every new finance table was auto-granted SELECT to anon** — the hole would have reopened itself), and enabled RLS on the backup table. **Scope was much wider than first reported: 25 tables carried `ALL TO public USING (true)`, not 3** — including `budgets` (2,148 rows), `categories`, `import_sessions`, `subscriptions`, `planning_notes`, `ai_usage_tracking`. Note three policies are *named* as though service-role-only ("Service role full access on subscriptions", "Allow full access via service key") but were actually `TO public`. **Two corrections to the original finding:** (1) writes were already blocked — anon held SELECT only, so DELETE/PATCH returned 401; this was a confidentiality leak, not an integrity risk. (2) `truelayer_connections` / `enable_banking_sessions` / `monthly_reports` were confirmed safe (RLS on, zero policies, 0 rows via anon) — no bank credentials were ever exposed. Separately found and closed an RPC bypass: `finance.add_monthly_manual_income()` was SECURITY DEFINER with a NULL proacl, so anon could POST to it and write a transaction regardless of table grants (date-gated + idempotent, so low impact); its real caller is pg_cron job 7 as `postgres`, unaffected. **Verified after applying:** anon → 401 on all 12 tables tested and on the RPC; service_role reads unchanged (transactions still 3,683); financial-data MCP `get_net_worth` returns full data end-to-end. **[UPDATE — same day] The "latent" `authenticated` follow-up was NOT latent, and is now also fixed** (migration `lock_finance_schema_to_service_role`). I called it latent by assuming `authenticated` meant Chris; `auth.users` actually holds **68 users, 58 of whom signed up in June 2026** (the football prediction game shares this project), so ~58 external people could read AND WRITE all 25 finance tables — worse than the anon leak, since `authenticated` held DELETE/TRUNCATE where anon had SELECT only. **Demonstrated, not inferred:** a throwaway confirmed user signed in via the ordinary anon-key flow, read transactions (3,683) / wealth_snapshots (813) / budgets (2,148), and got HTTP 204 (authorized, zero rows matched) on DELETE and PATCH; user deleted after, count back to 68, transactions unchanged at 3,683. Fixed by dropping all 25 policies (dynamically — three were misleadingly named as service-role-only), enabling RLS everywhere, and revoking `authenticated` grants + default privileges. Verified: same JWT now 403 on every table and both writes; service_role unchanged; MCP `get_net_worth` + `get_budget_status` both fine. Final state: 0 policies, 0 anon/authenticated grants, 0 tables without RLS. **Lesson: check `auth.users` before calling an `authenticated` exposure low-risk — on a shared project, one consumer app's signup flow widens every `TO public` policy in every other schema.** Remaining: **46 tables outside finance are still anon-readable** (`public` 31, `japan` 12, `practice` 3) incl. `price_snapshots` (2.7M rows Keepa/BL pricing), `energy_live` (62,701 rows minute-resolution = occupancy signal), `japan_bookings`, `practice.papers`. These have REAL anon consumers (IHD reads `energy_daily_summary` with the anon key) so they need per-table triage, not a blanket revoke. Also 3 ERROR `security_definer_view` in `public` — lowest priority, since the tables they read are already anon-readable.
